...
4. `fig_4.m`: This file simulates the invasion fitness of the mutant $\psi_{\sigma_{h2(r)}}(\sigma_{h2(m)})$ for different combinations of resident and mutant trait value, $(\sigma_{h2(r)},\sigma_{h2(m)}) \in [0,1] \times [0,1]$. The encounter rates of $h1$ and $h2$ are taken as ( $\mathcal{E}{h1},\mathcal{E}_{h2}$ ) $=(2,1),(1.05,1),(1,1.5),(1,5)$ for Figures (a)-(d), respectively. The value of the invasion fitness is stored in matrix `z` of size 500 $\times$ 500. The Parameters $p_2=0.95$, $q_2=0.95$, $\nu=0.5, \chi=1$, $\tau_2$=2.5 day, $\tau_f=0.1$ day, are taken from [C. Stone and K. Gross. 2018. Malar. J](https://doi.org/10.1186/s12936-018-2407-1).
