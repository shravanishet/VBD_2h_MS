...
R0=(c1*c2/d)*((a^(2)*Nh1/(mu1+d1))+(Nh2/(del2+d2)))

m3=a*c1*(1+(mu1/(d1+del1)));
m1=(mu1+d1);
m2=(d2+del2);

A=c1*c2*(a^2*Nh1*c1+Nh2*m3)+d*c1*m3;
C=d*m1*m2-c1*c2*(a^2*Nh1*m2+Nh2*m1);
B=d*(c1*m1+m2*m3)+c1*c2*(a^2*Nh1*(m2-c1)+Nh2*(m1-m3));


 disp('global stability >0')
 disp(d*(mu1+d1)-2*a^2*c1*c2*Nh1)
 disp(d^2*(mu1+d1)*(d2+del2)-a^2*c1^2*c2^2*Nh1*Nh2*4)
...
IH1=W(:,1); IH2=W(:,2); IV=W(:,3); Rh1=W(:,4); %SH1=W(:,5);
...
grid on
xlabel ('$I_{h1}$','Interpreter','latex','Rotation',0,'FontWeight','bold','FontName','Helvetica','FontSize',12);
ylabel ('$I_{v}$','Interpreter','latex','Rotation',0,'FontWeight','bold','FontName','Helvetica','FontSize',12);
zlabel ('$I_{h2}$','Interpreter','latex','Rotation',0,'FontWeight','bold','FontName','Helvetica','FontSize',12);
hold on 
...
text(Ih1,Iv,Ih2,'$(I_{h1}^{*},I_{v}^{*},I_{h2}^{*})$','Interpreter','latex','FontSize',12);
